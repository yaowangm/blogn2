...
class EditPostForm extends BaseComponent {
            ...
            if (articleData) {
                ...
                if (this.originalArticleData.attachment) {
                    ...
                    this.existingImage = {
                        url: imageUrl,
                        name: '现有主图片',
                        relative_path: this.originalArticleData.attachment
                    };
                } else {
                ...
                // 设置现有多张图片
                this.existingImages = [];
                if (this.originalArticleData.attachments && this.originalArticleData.attachments.length > 0) {
                    this.existingImages = this.originalArticleData.attachments.map(att => ({
                        id: att.id,
                        url: att.linkstr.startsWith('http') ? att.linkstr : `/upload/${att.linkstr}`,
                        name: att.comment || '图片附件',
                        relative_path: att.linkstr,
                        comment: att.comment || ''
                    }));
                }
    ...
    async removeImage(imageId = null) {
        if (imageId) {
            // 删除指定的图片
            ...
            if (imageToRemove) {
                // 如果是临时文件，立即删除服务器上的临时文件
                if (isFromUploaded && imageToRemove.is_temp) {
                    try {
                        await fetch(`/api/temp-upload/${imageToRemove.filename}`, {
                            method: 'DELETE'
                        });
                    } catch (error) {
                ...
                if (isFromExisting) {
                    ...
                    this.deletedImages.push(imageToRemove);
                }
                ...
                if (imageId === 'main') {
                    ...
                    if (this.uploadedImages.length > 0) {
                        ...
                        // 恢复现有主图
                        this.uploadedImage = null;
                        this.formData.attachment = this.existingImage.relative_path;
                    } else {
                        this.uploadedImage = null;
                        this.formData.attachment = null;
                    }
    ...
    updateImagePreview() {
        ...
        if (imageContainer) {
            ...
            if ((this.uploadedImage || this.existingImage) || allImages.length > 0) {
                    ...
                    <div class="uploaded-image-preview">
                        ${this.uploadedImage || this.existingImage ? `
                            <div class="main-image-preview">
                                <img src="${(this.uploadedImage || this.existingImage).url}" alt="主图片" class="preview-image">
                                <div class="image-info">
                                    <span class="image-name">${(this.uploadedImage || this.existingImage).name} (主图)</span>
                                    <span class="image-size">${this.uploadedImage ? (this.uploadedImage.size / 1024).toFixed(1) + 'KB' : '现有图片'}</span>
                                </div>
                                <button type="button" class="btn-remove-image" data-image-id="${(this.uploadedImage || this.existingImage).id || 'main'}" title="删除主图">
                                    ...
                                    ${[...this.existingImages, ...this.uploadedImages].map(img => `
                                        <div class="image-item">
                                            ...
                                            <div class="image-info">
                                                ...
                                                <span class="image-size">${img.size ? (img.size / 1024).toFixed(1) + 'KB' : '现有图片'}</span>
                                            </div>
                                            <button type="button" class="btn-remove-image" data-image-id="${img.id}" title="删除图片">×</button>
                                        </div>
                                    `).join('')}
                                </div>
                            </div>
                ...
                // 添加删除按钮的事件委托
                this.setupImageDeleteEvents();
            }
        }
    }
    
    setupImageDeleteEvents() {
        const imageContainer = this.shadowRoot.querySelector('.image-upload-container');
        if (imageContainer) {
            // 移除之前的事件监听器
            imageContainer.removeEventListener('click', this.handleImageDelete);
            
            // 添加新的事件监听器
            this.handleImageDelete = (event) => {
                const deleteBtn = event.target.closest('.btn-remove-image');
                if (deleteBtn) {
            ...
            imageContainer.addEventListener('click', this.handleImageDelete);
        }
    }
