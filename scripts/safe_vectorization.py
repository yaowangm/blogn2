...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('safe_vectorization.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SafeVectorization:
    """安全版批量向量化处理器"""
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='安全版存量数据向量化脚本')
    ...
    logger.info("=" * 60)
    logger.info("安全版存量数据向量化脚本启动")
    logger.info(f"清空向量表: {args.clear_tables}")
    logger.info(f"恢复模式: {args.resume}")
    logger.info("=" * 60)
    
    # 创建向量化处理器
    processor = SafeVectorization(clear_tables=args.clear_tables)
