...
class TestMetadataController:
    ...
    def test_get_site_metadata_success(self, client: TestClient):
        """测试成功获取网站元数据"""
        with patch('src.controllers.metadata.MetadataService') as mock_service:
            mock_service.return_value.get_metadata_dict = AsyncMock(
                return_value={
                    "total_users": 10,
                    "total_posts": 5,
                    "active_users": 8
...
class TestUserController:
    ...
    def test_get_user_summary_success(self, client: TestClient):
        """测试成功获取用户统计信息"""
        with patch('src.controllers.user.UserService') as mock_service:
