...
**显示内容：**
- 博客名称 + 24px 内联头像（与 `blog-list-card` 作者 meta 一致）
- 文章标题（自动截断，默认 40 字）
- 更新时间（相对时间格式）

**交互功能：**
- 整行点击跳转到文章页：`/article/{id}`
- 响应式设计和加载状态

**UI 细节**（见 [RECENT_UPDATES_CARD_UI_IMPROVEMENT.md](./RECENT_UPDATES_CARD_UI_IMPROVEMENT.md)）：
- 使用 `renderAuthorMetaItem(blog_name, avatar, userid)` 渲染头部
- 头像优先 API `avatar`，否则 `/avatar/{prefix}/s_{userid}.jpg`
