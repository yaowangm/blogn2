...
> **说明**：下表为历史逐条索引，新增模块（向量搜索、安全、静态资源、文章评论分页等）的用例未全部列入。以 `pytest --collect-only` 为准。

- **总测试数量**: 827（2026-06，`tests/unit` 677 + `tests/integration` 126 + 根目录/performance 22，另含新增缓存/评论用例）
- **测试文件**: 82 个 `test_*.py`
- **单元测试**: 677
- **集成测试**: 126

概览与运行方式见 [TESTING_SUMMARY.md](TESTING_SUMMARY.md)。
...
### 测试质量指标
- **测试密度**: 高（827 个用例，覆盖 controllers / services / repositories / 集成 API）
- **测试类型平衡**: 单元测试约 82% + 集成测试约 15% + 其他约 3%
- **测试策略**: Mock 单元测试 + 真实数据库集成测试 + 静态/性能专项测试

---

**最后更新**: 2026-06  
**测试总数**: 827（运行 `python -m pytest --collect-only -q` 验证）  
**测试文件**: 82  
**覆盖率**: 运行 `python -m pytest --cov=src` 查看当前值 